...
class CustomImageDisplayOptions {
  ...
  bool applyFibonacciDragWidget = true;
...
class CustomImageDisplay extends StatefulWidget {
  const CustomImageDisplay(
      {super.key, required this.options, required this.screenshotStream});
...
class CustomImageDisplayState extends State<CustomImageDisplay> {
  ...
  Widget build(BuildContext context) {
    if (!widget.screenshotStream.hasListener)
      widget.screenshotStream.stream.listen((event) {
        saveScreen();
      });

    final imgSizeFuture =
        _getDimensionsOfCurrentImage(); // Initiate async operations first, then do calculations
                    ...
                    ResizableWidget(
                        ...
                        dragWidget: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color: Color.fromARGB(
                                options.applyFibonacciDragWidget ? 200 : 0,
                                33,
                                149,
                                243),
                          ),
                        ...
                        child: RotationTransition(
                          turns: AlwaysStoppedAnimation(
                              options.fibonacciRotationDegrees / 360),
                          child: CustomPaint(
